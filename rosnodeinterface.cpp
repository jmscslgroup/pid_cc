...
// Generated 16-Apr-2021 11:29:37
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"pid_cc\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(pid_cc_M, (NULL));
        pid_cc_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(pid_cc_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(pid_cc_M)
                == (NULL)) || rtmGetStopRequested(pid_cc_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(pid_cc_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
